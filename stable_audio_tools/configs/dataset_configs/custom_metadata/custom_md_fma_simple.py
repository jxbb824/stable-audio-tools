...
_DATASET_DIR = Path(__file__).resolve().parents[4] / "dataset" / "small-5000"
_RAW_TSV = _DATASET_DIR / "raw_30s.tsv"
_AUDIO_METADATA_TSV = _DATASET_DIR / "raw.meta.tsv"
...
def _build_prompt(meta, raw):
    """Build simplified prompt with only lowercase mood, genre, instrument separated by commas."""
    tags_by_type = raw.get("tags_by_type", {}) if raw else {}
    
    all_tags = []
    
    # Collect mood, genre, instrument tags
    for tag_key in ["mood/theme", "genre", "instrument"]:
    ...
    prompt = ", ".join(tag.lower() for tag in all_tags)
    return prompt
